...
class CourseSyncActivities:
    ...
    async def create_sync_jobs(self) -> JobSyncResult:
        ...
        try:
            ...
            group_result = self.supabase.table("job_sync_groups").insert(job_sync_group_data).execute()
            ...
            # Get all sources (which contain the canvas URLs)
            sources_result = self.supabase.table("sources").select("*").execute()
            
            if not sources_result.data:
                activity.logger.warning("No sources found")
                return JobSyncResult(job_sync_ids=[], total_created=0)
            ...
            for source in sources_result.data:
                ...
                sync_result = self.supabase.table("job_syncs").insert(job_sync_data).execute()
    ...
    async def scrape_course(self, job_sync_id: str) -> ScrapeResult:
        ...
        try:
            ...
            job_sync_result = self.supabase.table("job_syncs").select("*").eq("id", job_sync_id).execute()
            ...
            source_result = self.supabase.table("sources").select("url").eq("id", source_id).execute()
            ...
            previous_tree = None
            previous_sync_result = self.supabase.table("job_syncs")\
                .select("scraped_tree")\
            ...
            if previous_sync_result.data and previous_sync_result.data[0]["scraped_tree"]:
                previous_tree = previous_sync_result.data[0]["scraped_tree"]
                activity.logger.info(f"Found previous tree for change detection from source: {source_url}")
            
            # Get user_id from job_sync_groups to find cookies
            group_id = job_sync["job_sync_group_id"] 
            group_result = self.supabase.table("job_sync_groups").select("user_id").eq("id", group_id).execute()
            ...
            if group_result.data and group_result.data[0]["user_id"]:
                ...
                cookies_result = self.supabase.table("user_auth_details").select("cookies").eq("user_id", user_id).execute()
    ...
    async def find_assignments(self, job_sync_id: str) -> AssignmentResult:
        ...
        try:
            ...
            course_assignments = self.supabase.table("assignments")\
                .select("id")\
                .eq("course_id", course_id)\
                .execute()
            
            assignment_ids = [a["id"] for a in course_assignments.data] if course_assignments.data else []
    ...
    @activity.defn
    async def find_due_dates(self, job_sync_id: str, assignment_ids: List[str] = None) -> DueDateResult:
        """
        ...
        try:
            ...
            if not assignment_ids:
                course_id = job_sync["course_id"]
                assignments_result = self.supabase.table("assignments")\
                    .select("*")\
                    .eq("course_id", course_id)\
                    .execute()
            ...
            if not assignments:
                activity.logger.warning(f"No assignments found for job sync {job_sync_id}")
